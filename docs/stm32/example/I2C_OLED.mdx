...
- 在Pinout&Configuration页面，Connectivity -> I2C1，将I2C模式选择为I2C，并在下方Configuration -> Parameter Settings，将I2C Speed Mode选择为Fast Mode

<img src={require('./img/I2C_OLED/Snipaste_2023-05-04_23-06-38.png').default} alt="工程配置" width="60%" />

- 在System Core -> RCC中，将High Speed Clock(HSE)配置为Crystal/Ceramic Resonator
- 切换到Clock Configuration页面，将HCLK（MHz）改为72，并按回车键。

<img src={require('./img/I2C_OLED/Snipaste_2023-05-04_18-27-04.png').default} alt="工程配置" width="60%" />

- 在Project Manager -> Code Generator页面中，勾选Generate peripheral initialization as ... per peripheral
...
- 中文字符：中文字符数量多，STM32内部Flash无法全部存储，因此需要根据使用的字符来取字模。
  - 在线取字模工具和教程完善中，**即将更新**，请关注Keysking公众号推送。

**图片：**支持BMP格式单色位图

- 该工具同上，**即将更新**，请关注Keysking公众号推送。
...
- **cube重新生成代码后，中文出现乱码：**这是cubeIDE的问题，我们在环境变量中添加一行配置即可解决（仅Windows下）
  ...
  <img src={require('./img/I2C_OLED/搜索环境变量.png').default} alt="搜索环境变量" width="60%" />
  ...
  <img src={require('./img/I2C_OLED/添加环境变量.png').default} alt="添加环境变量" width="60%" />
...
<img src={require('./img/I2C_OLED/勾选生成独立初始化文件.png').default} alt="勾选生成独立初始化文件" width="68%" />

- **打开.ioc配置文件时弹出对话框”New STM32Cube firmware version available“：**因为您的软件版本比例程高，此时建议直接点击”Migrate“迁移到您的版本即可，以后再打开就不会出现了。

<img src={require('./img/I2C_OLED/New version dialog.png').default} alt="New version dialog" width="60%" />

### 程序下载问题

- **下载程序时，提示如图错误：Failed to start GDB server** 

<img src={require('./img/I2C_OLED/错误提示.png').default} alt="错误提示" width="67%" />
...
<img src={require('./img/I2C_OLED/选择debug模式.png').default} alt="选择debug模式" width="60%" />

- **下载程序时，提示“需要升级ST-LINK固件”，如图所示**

<img src={require('./img/I2C_OLED/需要升级STLINK固件.png').default} alt="需要升级STLINK固件" width="60%" />
